...
class RayShopeeMemoryService:
    ...
    async def add_project_context(self, content: str, tags: List[str] = None, **kwargs) -> Dict[str, Any]:
        """
        ...
        result = await self.memory.add(
            ...
            tags=tags or ["project-context"],
            meta=meta
        )
        return result

    async def search_context(self, query: str, limit: int = 10, category: str = None) -> List[Dict[str, Any]]:
        """
    ...
    async def add_architecture_decision(self, decision: str, context: str, alternatives: List[str] = None):
        """
...
async def main():
    ...
    parser.add_argument(
        "--user",
        help="User ID (default: ai-assistant)",
        default="ai-assistant"
    )
    parser.add_argument(
        "--tags",
        help="Tags for new entries (comma-separated)",
        metavar="TAGS"
    )
    parser.add_argument(
        "--category",
        help="Category for new entries",
        metavar="CATEGORY"
    )
